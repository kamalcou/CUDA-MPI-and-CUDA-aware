...
// t1_gpu = gettime();
  for (k = 0; k < NTIMES; k += 2) {
    ...
    MPI_Recv(&d_life[(myN+1)*(N+2)+0], N+2, MPI_INT, next, 0, MPI_COMM_WORLD,MPI_STATUS_IGNORE);
    MPI_Send(&d_life[1*(N+2)+0], N+2, MPI_INT, prev, 0, MPI_COMM_WORLD);
    
    MPI_Recv(&d_life[0*(N+2)+0], N+2, MPI_INT, prev, 0, MPI_COMM_WORLD,MPI_STATUS_IGNORE);
    MPI_Send(&d_life[myN*(N+2)+0], N+2, MPI_INT, next, 0, MPI_COMM_WORLD);
    ...
    MPI_Recv(&d_temp[(myN+1)*(N+2)+0], N+2, MPI_INT, next, 0, MPI_COMM_WORLD,MPI_STATUS_IGNORE);
    MPI_Send(&d_temp[1*(N+2)+0], N+2, MPI_INT, prev, 0, MPI_COMM_WORLD);
    
    MPI_Recv(&d_temp[0*(N+2)+0], N+2, MPI_INT, prev, 0, MPI_COMM_WORLD,MPI_STATUS_IGNORE);
    MPI_Send(&d_temp[myN*(N+2)+0], N+2, MPI_INT, next, 0, MPI_COMM_WORLD);
...
if(rank==0)
    printf("Cuda aware GPU time is taken=%f ms\n",ms1);
...
//   }
  ...
  if (rank == 0) {
    ...
    sprintf(filename,"%s/cuda_aware_MPI_output.%d.%d.%d", argv[3], N, NTIMES, size);
